...
function maverick_setup() {
	 ...
	 * If you're building a theme based on Maverick, use a find and replace
...
function maverick_scripts() {
	...
	wp_enqueue_style( 'mystyle', get_template_directory_uri() .'/styleme.css');
	
	$fonts_url='http://fonts.googleapis.com/css?family=Lato:300,400,700,300italic,400italic|Open+Sans:300italic,400italic,400,300,700|Roboto:400,300,300italic,400italic,700';
	if ( !empty($fonts_url)){
		wp_enqueue_style('font-name',esc_url_raw($fonts_url),array(),null);
	}
	
	$fonts_url='http://fonts.googleapis.com/css?family=Muli:300,400,300italic,400italic|Pacifico|Playball|Lobster+Two:400,400italic,700';
	if ( !empty($fonts_url)){
		wp_enqueue_style('font-name',esc_url_raw($fonts_url),array(),null);
	}

	wp_enqueue_script( 'maverick-navigation', get_template_directory_uri() . '/js/navigation.js', array(), '20120206', true );

	wp_enqueue_script( 'maverick-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20130115', true );
	
	wp_enqueue_script( 'javascript', get_template_directory_uri() . '/js/bootstrap.min.js');
	
	wp_enqueue_script( 'bootjavascript', get_template_directory_uri() . '/js/bootstrap-wp.js');
